...
class Solution {
    public int mincostTickets(int[] days, int[] costs) {
        ...
        for (int i = 1; i < size; i++) {
            int buyWeek, buyMonth;
            int lastWeek = i - 1, lastMonth = i - 1;

            //检索上次购买 月卡/周卡的时间
            while (lastWeek >= 0 && days[i] - days[lastWeek] < 7) {
                lastWeek--;
            }
            while (lastMonth >= 0 && days[i] - days[lastMonth] < 30) {
                lastMonth--;
            }

            buyWeek = lastWeek == -1 ? costs[1] : dp[lastWeek] + costs[1];
            buyMonth = lastMonth == -1 ? costs[2] : dp[lastMonth] + costs[2];

            dp[i] = Math.min(dp[i - 1] + dp[0], Math.min(buyWeek, buyMonth));
        }
        return dp[size - 1];
    }
